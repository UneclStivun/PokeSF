...
<%-- 
  ...
  - Description: pokemonTeamCreator.jsp
  --%>
<!DOCTYPE html>
<html>
<head>
		...
		<div class="col-5">
			<div class="d-flex p-2">
				<div class="d-inline-flex p-2">
					<form action="ServletCreateTeam" method="post">
						<c:if test="${sessionScope.quickList != null && !sessionScope.quickList.isEmpty()}">
							<%int number = 0;%>
								...
								<c:forEach items="${sessionScope.quickList}" var="result">
									<tr>
										<td><button type="button" class="btn btn-link"
												onclick="showAttacks('q${result.getDatabaseID()}')">${result.getName()}</button></td>
										<td>${result.getType1()}</td>
										<td>${result.getType2()}</td>
										<td>${result.getHitpoints()}</td>
										<td>${result.getAttack()}</td>
